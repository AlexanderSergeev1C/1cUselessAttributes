...
&НаСервереБезКонтекста
Функция ПолучитьШаблонСтрокиТабличныхДанных(ЭтоДанныеСтроковогоТипа)
	
	Если ЭтоДанныеСтроковогоТипа Тогда
		Строка = "
			|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ПОДСТРОКА(%ИмяМетаданных%.%ИмяТабличнойЧасти%.%ИмяРеквизита%, 1, 1024)) КАК КоличествоРазличныхЗначений_%ИмяРеквизита%,
			|	МАКСИМУМ(ПОДСТРОКА(%ИмяМетаданных%.%ИмяТабличнойЧасти%.%ИмяРеквизита%, 1, 1024)) КАК МаксимальноеЗначение_%ИмяРеквизита%";
	Иначе
		Строка = "
			|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ %ИмяМетаданных%.%ИмяТабличнойЧасти%.%ИмяРеквизита%) КАК КоличествоРазличныхЗначений_%ИмяРеквизита%,
			|	МАКСИМУМ(%ИмяМетаданных%.%ИмяТабличнойЧасти%.%ИмяРеквизита%) КАК МаксимальноеЗначение_%ИмяРеквизита%";
	КонецЕсли;
